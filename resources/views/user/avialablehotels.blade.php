...
<div class="d-flex justify-content-center mt-4">
    <div class="container">
        <div class="row col-12 mx-auto">
            ...
            @foreach($availableRooms as $wishlistItem)
            <div class="col-lg-3 col-md-3 col-sm-12">
                    <div class="card border-0 mt-3">
                        <div class="rounded-square">
                            <a href="{{ route('hotel.singal', ['id' => $wishlistItem->roomType->hotel->id]) }}" class="mt-5" style="text-decoration: none; color: inherit;">

                            <img src="{{ asset($wishlistItem->roomType->hotel->images->first()->image_path) }}" class="equal-size rounded-square-img" alt="best-service">
                            </a>
                        </div>
                        <div class="" id="slide">
                            <div class="heart bg-light p-2" style="border-radius:50%;">
                                @if(in_array($wishlistItem->roomType->hotel->id, $userWishlist))
                                    <i class="fa fa-heart rounded-circle bg-light p-1 like-button liked" data-hotel-id="{{ $wishlistItem->roomType->hotel->id }}" aria-hidden="true"></i>
                                @else
                                    <i class="fa fa-heart-o rounded-circle bg-light p-1 like-button" data-hotel-id="{{$wishlistItem->roomType->hotel->id }}" aria-hidden="true"></i>
                                @endif
                            </div></div>
                    </div>
                    <a href="{{ route('hotel.singal', ['id' => $wishlistItem->roomType->hotel->id]) }}" class="mt-5" style="text-decoration: none; color: inherit;">

                    <h5 class="mt-3">{{ ucwords($wishlistItem->roomType->hotel->name) }}</h5>
        <p class="mt-1">{{ \App\Models\City::find($wishlistItem->roomType->hotel->city)->name }}</p>
                    </a>
                    <div class="d-flex align-items-center mt-3">
                        <div>
                            <button type="button" class="btn" style="background-color:#FE3400;color: white;">{{ $wishlistItem->roomType->hotel->stars}}.0</button>
                        </div>
